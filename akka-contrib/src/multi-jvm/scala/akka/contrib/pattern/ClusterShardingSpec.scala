  ...
  "Persistent Cluster Shards" must {
    ...
    "permanently stop entries which passivate" in within(15.seconds) {
      ...
      runOn(third) {
        ...
        //Possible race condition within the test; it may be possible for the hand off
        // message to be received before the Shard receives the Terminated message
        // If the shard receives the terminated after it starts the handoff, it assumes
        // the entry stopping was triggered by the hand off, not a passivate.
        Thread sleep 500
